...
def eval_poly(f, x):
    ...
    for (degree,coeff) in reversed(list(enumerate(f))):
        y = y * x + coeff
    return y
...
def lagrange_polynomial(Poly, xs, xi):
    ...
    for xj in xs:
        if xj != xi:
            p_c = 1/(Poly.field(xi)-Poly.field(xj))
            p_l = Poly.field(-1) * Poly.field(xj) * p_c
            result *= Poly([p_l, p_c])
    return result
...
def interpolate(Poly, points):
    ...
    # TODO: Your code goes here
    for x,y in points:
        f += lagrange_polynomial(Poly, xs, x) * Poly.field(y)
    return f
...
if __name__ == '__main__':
    ...
    print ("1. exam: ", interpolate(Poly, [[1, 37], [2, 47], [3, 33], [4, 51]]))
    print ("2. exam: ", interpolate(Poly, [[0, 0], [1, 37]]))
    ...
    poly3_ = interpolate(Poly, zip(xs, ys))
